...
/*   Updated: 2024/01/12 18:21:51 by kyusulee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	**ft_alloc_word(const char *s, const char *set, char **results)
{
	size_t		i;
	size_t		j;
	int			can_cnt_word;
...
char	**ft_split_set(char const *s, char *set)
{
	...
	if (s == NULL)
		return (NULL);
	len = ft_get_arrlen(s, set);
	results = malloc(sizeof(char *) * (len + 1));
	if (!results)
		return (NULL);
	results[len] = NULL;
	results = ft_alloc_word(s, set, results);
	if (!results)
		return (NULL);
	return (results);
}
